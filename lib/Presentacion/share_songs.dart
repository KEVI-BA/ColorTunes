...
import 'package:colortunes_beta/Datos/share_songs.dart';
...
class _SharedSongsPageState extends State<SharedSongsPage>
  ...
  // Colores dinámicos para el gradiente
  ...
  void dispose() {
    ...
    _controller.dispose();
    _colorChangeTimer?.cancel(); // Cancelar el timer al destruir la pantalla
    super.dispose();
  }
  ...
  void initState() {
    ...
    // Iniciar el cambio de colores cada 2 segundos
    _colorChangeTimer = Timer.periodic(const Duration(seconds: 2), (_) {
      _changeGradientColors();
    });
  }

  // Método para cambiar los colores del gradiente
